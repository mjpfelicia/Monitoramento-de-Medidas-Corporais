...
const ComposicaoCorporal = () => {
    ...
    // Função para calcular a porcentagem de gordura corporal (exemplo simplificado)
    const calcularComposicaoCorporal = () => {
        if (peso && altura && cintura) {
            const imcValue = calcularIMC();
            const gorduraCorporalValue = (cintura / peso) * 100;  // Apenas um exemplo simplificado
            setImc(imcValue);
            setGorduraCorporal(gorduraCorporalValue);
            return `IMC: ${imcValue}, Gordura Corporal Estimada: ${gorduraCorporalValue.toFixed(2)}%`;
        }
        return null;
    };
    ...
    const getIMCIcon = () => {
        if (imc) {
            ...
            if (imc >= 25 && imc < 29.9) return <i className="fas fa-exclamation-triangle" style={{ color: '' }}></i>; // Sobrepeso
    ...
    return (
        <div className="composicaoCorporal-container">
            ...
            <form className="composicaoCorporal-form">
                ...
                <input className="composicaoCorporal-input" type="number" value={peso} onChange={(e) => setPeso(e.target.value)} />

                <label className="composicaoCorporal-label">Altura (m):</label>
                <input
                    className="composicaoCorporal-input" type="number" step="0.01" value={altura} onChange={(e) => setAltura(e.target.value)} />

                <label className="composicaoCorporal-label">Cintura (cm):</label>
                <input className="composicaoCorporal-input" type="number" value={cintura} onChange={(e) => setCintura(e.target.value)} />
            </form>

            <button className="composicaoCorporal-button" onClick={() => setResultado(calcularComposicaoCorporal())}>
